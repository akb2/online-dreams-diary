...
export class DreamMapWillowTreeObject extends DreamMapObjectTemplate implements DreamMapObjectTemplate {


  private type: string = "tree-willow";
  ...
  private minGeneration: number = 2;
  private maxGeneration: number = 4;
  private radiusSegments: number = 3;
  private segmentsCount: number = 8;
  ...
  private getBranchGeometry(lX: number, lY: number, scale: number, rotate: number, z: number, geometryIndex: number): MapObject {
    ...
    return {
      type,
      subType: DreamMapWillowTreeObject.getSubType(this.ceil, this.neighboringCeils, type, geometryIndex.toString()),
      splitBySubType: true,
      count: 1,
      matrix: matrix,
      skews: [],
