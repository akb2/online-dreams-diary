...
export const LightMapName: string = "lightMap";
...
const LightMap: string[] = CreateNamedArray(LightMapName);
...
export const TerrainFragmentShader: string = `
    ...
    .replace("#include <lightmap_pars_fragment>", `
          uniform sampler2D ${LightMapName};
    ...
    .replace("#include <lightmap_fragment>", `
          #ifdef USE_LIGHTMAP
            ${LightMap.map((n, k) => `vec4 ${n} = getTileTexture(${LightMapName}, ${MapTileCoords[k]}, vUv2);`).join("\n")}
